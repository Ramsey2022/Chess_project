...
class Pawn < Piece
  ...
  def find_possible_moves(board)
    [single_move(board), double_move(board)].compact
  end
  ...
  def double_move(board)
    ...
    return bonus unless invalid_double_move?(board, bonus)
  end

  def invalid_double_move?(board, bonus)
  ...
  def legal_en_passant_move?(board)
    ...
    temp_board = remove_captured_en_passsant_pawn(board, pawn_location)
    legal_capture = remove_illegal_moves(temp_board, en_passant_move)
    legal_capture.size.positive?
  end

  def remove_captured_en_passsant_pawn(board, pawn_location)
